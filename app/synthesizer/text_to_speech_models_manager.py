...
class TextToSpeechModelsManager:
    def __init__(self, models_folder_path):
        ...
        self.models = { 
            Language.POL: {
                "vits": TextToSpeechVitsModel("vits", Language.POL, models_folder_path + "/model/vits"),
            },
            Language.ENG: {
                "vits": TextToSpeechVitsModel("vits", Language.ENG, models_folder_path + "/model/vits"),
                "speecht5": TextToSpeechSpeechT5Model("speecht5", Language.ENG,models_folder_path + "/model/speecht5/model", models_folder_path + "/model/speecht5/processor", models_folder_path + "/vocoder/hifigan"),
            }
        }
...
def get_text_to_speech_models_manager() -> TextToSpeechModelsManager:
    ...
    if _text_to_speech_models_manager is None:
        with _text_to_speech_models_manager_lock:  
            if _text_to_speech_models_manager is None:  
                _text_to_speech_models_manager = TextToSpeechModelsManager("./download")
